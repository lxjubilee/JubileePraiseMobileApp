...
**Steps:**
1. Trigger `jubileepraise://album/<CODE>`.
...
**Steps:**
1. Open `https://jubileepraise.com/album?c=<CODE>` (and `?code=<CODE>`).
...
**Preconditions:** Signed in.
**Steps:**
1. Open `https://jubileepraise.com/album/<CODE>`.
...
**Preconditions:** Signed in.
**Steps:**
1. Open `jubileepraise://home`, `/browse`, `/search`, `/library`.
...
**Preconditions:** Signed in.
**Steps:**
1. Open `jubileepraise://artist/<ID>`.
...
**Steps:**
1. Tap Share.
**Expected Result:** Native share sheet with `https://jubileepraise.com/album?c=<CODE>` (iOS
...
**Preconditions:** iOS; app installed; AASA served on jubileepraise.com.
**Steps:**
1. Tap an `https://jubileepraise.com/album/...` link from another app.
**Expected Result:** Opens directly in the app via associated domains (`applinks:jubileepraise.com`,
`applinks:www.jubileepraise.com`). *Known dependency: requires the `.well-known/AASA` file
...
**Steps:**
1. Tap an `https://jubileepraise.com/album/...` link.
**Expected Result:** Opens the app directly via the `autoVerify` intent filter (host
jubileepraise.com/www, `pathPrefix /album`). *Known dependency: requires
...
**Preconditions:** Signed in.
**Steps:**
1. Open `jubileepraise://album/` (no code), a bad code, and an unrelated path.
