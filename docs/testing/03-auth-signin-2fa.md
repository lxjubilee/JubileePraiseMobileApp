...
**Preconditions:** Signed out, on the Jubilee Door; an email with an existing JubileePraise account; Online.
**Steps:**
...
JubileePraise account. In `local`/`ji` mode `existsInSso === existsLocally`, so this branch is
...
and the door advances to **Create your JubileePraise account** with first/last name and date of
...
3. From **Confirm it's you** → **Create your JubileePraise account**, press back.
