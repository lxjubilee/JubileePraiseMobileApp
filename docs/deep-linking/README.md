# Deep linking — open shared album links in the JubileePraise app

**Goal:** tapping `https://jubileepraise.com/album?c=<CODE>` opens the installed app on the
...
- `app.json` → `ios.associatedDomains`: `applinks:jubileepraise.com`, `applinks:www.jubileepraise.com`
- `app.json` → `android.intentFilters`: `autoVerify: true` for `https://(www.)jubileepraise.com/album*`
- `app.json` → `scheme: "jubileepraise"` (custom-scheme fallback: `jubileepraise://album/<CODE>`)
...
hosted on jubileepraise.com. Without them the OS cannot verify the domain→app link and opens
the website instead of the app. These files must be served from the **jubileepraise.com web
app** (this repo only holds the ready-to-deploy copies as a hand-off).

## What to deploy on jubileepraise.com
...
| --- | --- |
| `apple-app-site-association` (no extension) | `https://jubileepraise.com/.well-known/apple-app-site-association` |
| `assetlinks.json` | `https://jubileepraise.com/.well-known/assetlinks.json` |

Also serve both on `https://www.jubileepraise.com/.well-known/...` (the app associates the
`www` host too).
...
1. **`apple-app-site-association` → `appID`** = `<AppleTeamID>.com.jubileepraise.app`
...
- **Google Statement List Tester:** paste
  `https://digitalassetlinks.googleapis.com/v1/statements:list?source.web.site=https://jubileepraise.com&relation=delegate_permission/common.handle_all_urls`
  → should return your `assetlinks.json` statement.
- **Apple AASA validator:** `https://app-site-association.cdn-apple.com/a/v1/jubileepraise.com`
  (Apple's CDN cache) or any AASA validator → should show the `/album` paths.
- **Android device (app installed):**
  `adb shell am start -a android.intent.action.VIEW -d "https://jubileepraise.com/album?c=JEIM1071EN"`
  → opens the app on the album. Check verification with
  `adb shell pm get-app-links com.jubileepraise.app` (domain should be `verified`).
