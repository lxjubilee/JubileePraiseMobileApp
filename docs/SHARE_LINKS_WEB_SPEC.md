# Share Links — JubileePraise.com web/backend spec (hand-off)
...
experience, the **JubileePraise.com** repo (`D:\Projects Data\JubileePraise.com`, Next.js web in
`app/web`) needs the three pieces below. The mobile side is already done.

## Canonical share URL
```
https://jubileepraise.com/album?c=<ALBUM_CODE>
```
...
| **Play Store URL** | `https://play.google.com/store/apps/details?id=com.jubileepraise.app` | store redirect |
| **Android signing SHA-256** | `eas credentials` → Android → Keystore (or Play App Signing cert) | `assetlinks.json` |

Bundle id / package is `com.jubileepraise.app` (both platforms).
...
export function generateMetadata({ searchParams }): Metadata {
  ...
  const url = `https://jubileepraise.com/album?c=${album.code}`;
  const image = `https://jubileepraise.com/cover/${album.code}.png`;
  const title = `${album.title} — ${album.artistName}`;
  const description =
    `${album.title} by ${album.artistName}. ${album.trackCount} tracks. Listen on JubileePraise.`;

  return {
    title,
    description,
    openGraph: {
      title, description, url, siteName: 'JubileePraise.com',
      type: 'music.album',
...
export function GET() {
  return Response.json({
    applinks: {
      apps: [],
      details: [
        { appID: '<APPLE_TEAM_ID>.com.jubileepraise.app', paths: ['/album', '/album/*'] },
      ],
    },
  });
}
  ...
  return Response.json([
    {
      ...
      target: {
        ...
        package_name: 'com.jubileepraise.app',
...
```
Verify after deploy:
- `https://jubileepraise.com/.well-known/apple-app-site-association` → 200, JSON, `application/json`.
- `https://jubileepraise.com/.well-known/assetlinks.json` → 200, JSON.
...
const PLAY_STORE = 'https://play.google.com/store/apps/details?id=com.jubileepraise.app';
...
```ts
other: { 'apple-itunes-app': 'app-id=<APP_STORE_ID>, app-argument=https://jubileepraise.com/album?c=<code>' }
```
...
- iOS Universal Links and Android App Links only **verify** once these files are live on
  `jubileepraise.com` **and** the app is rebuilt via EAS (the app already declares
