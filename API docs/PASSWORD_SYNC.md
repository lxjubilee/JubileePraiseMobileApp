# Password sync: JubileePraise → JubileeInspire (server-to-server)

When a user changes or resets their password on JubileePraise, the new password must
...
curl -X POST https://api.jubileeinspire.com/api/auth/service/token \
  ...
  -d '{"client_id":"jubileepraise","client_secret":"<the-shared-secret>"}'
...
password change. (Mirrors how JI already calls JubileePraise — see `AUTH_API 1.md`
§11–§12.)
